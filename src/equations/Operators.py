...
class Operators:
    ...
    def tf_trace(lt):
        ...
        # Optimization: Use tf.add_n instead of Python's sum() to avoid intermediate Python sum objects
        return tf.expand_dims(tf.add_n([v[:,i] for i, v in enumerate(lt)]) if lt else tf.constant(0.0, dtype=tf.float32), axis=-1)
