...
class LossNN(PhysNN):
    ...
    def metric_total(self, dataset, full_loss = True):
        ...
        # Performance optimization: tf.add_n is much faster than python sum() for accumulating tensors
        pst["Total"] = tf.add_n(list(pst.values())) if pst else tf.constant(0.0)
        llk["Total"] = tf.add_n(list(llk.values())) if llk else tf.constant(0.0)
        return pst, llk
    ...
    def loss_total(self, dataset, full_loss = True):
        ...
        # Performance optimization: tf.add_n is much faster than python sum() for accumulating tensors
        return tf.add_n(list(llk.values())) if llk else tf.constant(0.0)
