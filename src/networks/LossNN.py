...
class LossNN(PhysNN):
    ...
    def metric_total(self, dataset, full_loss = True):
        ...
        pst["Total"] = tf.add_n(pst_vals) if pst_vals else tf.constant(0.0)
        llk["Total"] = tf.add_n(llk_vals) if llk_vals else tf.constant(0.0)
        return pst, llk
    ...
    def loss_total(self, dataset, full_loss = True):
        ...
        llk_vals = list(llk.values())
        return tf.add_n(llk_vals) if llk_vals else tf.constant(0.0)
