...
class LossNN(PhysNN):
    ...
    def metric_total(self, dataset, full_loss = True):
        ...
        # Using tf.add_n instead of sum() to prevent creating numerous intermediate
        # Python objects during tensor summation, which is measurably faster.
        pst["Total"] = tf.add_n(list(pst.values())) if pst else self.tf_convert(0.0)
        llk["Total"] = tf.add_n(list(llk.values())) if llk else self.tf_convert(0.0)
        return pst, llk
    ...
    def loss_total(self, dataset, full_loss = True):
        ...
        # Using tf.add_n instead of sum() to prevent creating numerous intermediate
        # Python objects during tensor summation, which is measurably faster.
        return tf.add_n(list(llk.values())) if llk else self.tf_convert(0.0)
