...
class LossNN(PhysNN):
    ...
    def metric_total(self, dataset, full_loss = True):
        ...
        pst_vals = list(pst.values())
        pst["Total"] = tf.add_n(pst_vals) if pst_vals else tf.constant(0.0, dtype=tf.float32)

        llk_vals = list(llk.values())
        llk["Total"] = tf.add_n(llk_vals) if llk_vals else tf.constant(0.0, dtype=tf.float32)
    ...
    def loss_total(self, dataset, full_loss = True):
        ...
        llk_vals = list(llk.values())
        return tf.add_n(llk_vals) if llk_vals else tf.constant(0.0, dtype=tf.float32)
