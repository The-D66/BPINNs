...
class Theta():
    ...
    def ssum(self): 
        ...
        # Performance optimization: use tf.add_n and tf.reduce_sum(tf.square())
        # which is significantly faster than python sum() over tf.norm()**2
        return tf.add_n([tf.reduce_sum(tf.square(t)) for t in self.values])
    def size(self): 
        """ Counter of all entries of self.values """
        # Performance optimization: use math.prod for small, simple tuples (like tensor shapes)
        # to avoid numpy type conversion overhead
