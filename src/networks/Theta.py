...
class Theta():
    ...
    def ssum(self): 
        ...
        # Performance: tf.add_n(tf.reduce_sum(tf.square)) is faster than sum(tf.norm**2)
        return tf.add_n([tf.reduce_sum(tf.square(t)) for t in self.values]) if self.values else 0.0
    def size(self): 
        """ Counter of all entries of self.values """
        # Performance: math.prod is faster than np.prod for small iterables like shapes
