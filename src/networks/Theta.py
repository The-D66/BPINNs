...
class Theta():
    ...
    def ssum(self): 
        ...
        # Performance optimization: tf.add_n + tf.reduce_sum + tf.square is much faster
        # than using Python's sum() with tf.norm()**2 for a list of tensors.
        return tf.add_n([tf.reduce_sum(tf.square(t)) for t in self.values])
    def size(self): 
        """ Counter of all entries of self.values """
        import math
        # Performance optimization: Python's math.prod is faster than np.prod for small tuples
        # like tensor shapes because it avoids unnecessary NumPy array conversion overhead.
