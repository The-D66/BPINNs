...
class Theta():
    ...
    def ssum(self): 
        ...
        # Performance optimization: tf.add_n and tf.square are faster than python sum() with tf.norm()**2
        return tf.add_n([tf.reduce_sum(tf.square(t)) for t in self.values])
    def size(self): 
        """ Counter of all entries of self.values """
        # Performance optimization: math.prod is faster than np.prod for small tuple elements
