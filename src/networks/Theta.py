...
class Theta():
    ...
    def ssum(self): 
        ...
        # Optimized: tf.add_n and tf.reduce_sum(tf.square()) avoids sqrt calculation overhead in tf.norm
        return tf.add_n([tf.reduce_sum(tf.square(t)) for t in self.values])
    def size(self): 
        """ Counter of all entries of self.values """
        # Optimized: math.prod is faster than np.prod for small simple tuples like shapes
