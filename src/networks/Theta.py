...
class Theta():
    ...
    def ssum(self): 
        ...
        # Bolt optimization: tf.add_n + tf.reduce_sum(tf.square) is faster than sum + tf.norm**2
        return tf.add_n([tf.reduce_sum(tf.square(t)) for t in self.values])
    def size(self): 
        """ Counter of all entries of self.values """
        # Bolt optimization: math.prod avoids NumPy type conversion overhead for small tuples
