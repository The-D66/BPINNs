...
class Theta():
    ...
    def ssum(self): 
        ...
        Optimized to use tf.add_n and tf.reduce_sum(tf.square()) for faster execution
        than Python's sum() and tf.norm()**2.
        """
        return tf.add_n([tf.reduce_sum(tf.square(t)) for t in self.values])
    def size(self): 
        """ Counter of all entries of self.values
        Optimized to use math.prod instead of np.prod to avoid type conversion overhead.
        """
