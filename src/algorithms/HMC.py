...
class HMC(Algorithm):
    ...
    def sample_theta(self, theta_0):
        ...
        # Optimization: Pre-compute the initial gradient to avoid redundant evaluations
        # during leapfrog integration. The gradient computed at the end of step i
        # is passed directly as the initial gradient for step i+1, reducing evaluations from 2L to L+1.
        self.model.nn_params = theta
        grad_theta = self.model.grad_loss(self.data_batch, self.__full_loss)

        for _ in range(self.HMC_L):
            theta, r, grad_theta = self.__leapfrog_step(theta, r, self.HMC_dt, grad_theta)
