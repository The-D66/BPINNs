...
class HMC(Algorithm):
    ...
    def __leapfrog_step(self, old_theta, r, dt, grad_theta):
    ...
    def sample_theta(self, theta_0):
        ...
        # ⚡ Bolt Optimization: Cache the initial gradient to avoid redundant calculations.
        # By passing the gradient calculated at the end of the previous leapfrog step
        # to the next step, we reduce total gradient evaluations from 2L to L+1.
        self.model.nn_params = theta
        grad_theta = self.model.grad_loss(self.data_batch, self.__full_loss)
